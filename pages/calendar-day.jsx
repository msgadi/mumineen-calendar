...
const CalendarDay = (props) => { 
    ...
    const isToday=() => {
     ...
     const iconClassName = () => {
          var day = props.day, firstMiqaat = Lazy(props.miqaats)
                .filter({date: day.hijri.date})
                .pluck('miqaats')
                .flatten()
                .filter((miqaat) =>{ return miqaat.year ? (miqaat.year <= day.hijri.year) : true; }).first();
       ...
       const gregorianDateString=() => {
          let day = props.day.gregorian, dateString = day.date.toString();

          if (!props.day.filler) {
            if (props.day.hijri.date === 1 || day.date === 1) {  
              dateString += " " + new Date().getMonth() //.getShortMonthName() //getShortMonthName(day.month); - TODO: fix this
            }
      ...
      const onDayClick=  (day) =>{
          ...
          return ( <td className="box-border h-32 w-32 p-4 border-4 box-decoration-slice bg-gradient-to-r from-indigo-600 to-pink-500 text-white px-2" onClick={ () => onDayClick(null,  props.day)}>
              <div className="hijri">
                { hijriDateString()}
              </div>
